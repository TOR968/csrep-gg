...
const CONTAINER = '.csrep-gg-container';
...
async function detectActiveInjector(): Promise<Injector> {
	...
	try {
		...
		console.error('[CSrep] detect injector failed:', e);
	}
	return 'none';
}
...
async function setupCommunityInjection() {
	...
	const injectIntoTarget = (targetId: string) => {
		...
		return evalInTarget(CDP, targetId, buildInjectionCode(openExternal, CSREP_LOGO_DATA_URL), { awaitPromise: true });
	};

	reloadOpenProfiles = async () => {
		for (const t of await getProfileTargets(CDP)) {
			try { await evalInTarget(CDP, t.targetId, 'location.reload()'); }
			catch (e) { console.error('[CSrep] reload error:', e); }
		}
	};
	...
	const processTarget = (targetInfo: any) => {
		...
		pending.set(targetId, setTimeout(() => {
			...
			injectIntoTarget(targetId).catch((e: any) => console.error('[CSrep] injection error:', e));
		}, 200));
	};
	...
	return (
		<div style={{ padding: '16px' }}>
			...
			<div style={{ fontSize: '12px', lineHeight: '1.4', color: '#969696' }}>
				Open CSrep.gg in your system browser instead of Steam's built-in browser. Open profile pages reload automatically when you change this.
			</div>
		</div>
	);
};
